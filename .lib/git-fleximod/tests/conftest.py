...
def test_repo(shared_repos, tmp_path, logger):
    ...
    if "sparse" in submodule_name:
        ...
        gitp.git_operation(
            ...
            "--name",
            submodule_name,
            "https://github.com/ESMCI/mpi-serial.git",
            subrepo_path,
        )
...
def complex_repo(tmp_path, logger):
    ...
    gitp = GitInterface(str_path, logger)
    gitp.git_operation(
        "remote", "add", "origin", "https://github.com/jedwards4b/fleximod-test2"
    )
...
@pytest.fixture
def git_fleximod():
    def _run_fleximod(path, args, user_input=None):
