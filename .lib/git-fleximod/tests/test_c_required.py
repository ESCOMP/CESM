...
def test_required(git_fleximod, test_repo, shared_repos):
    ...
    status = git_fleximod(test_repo, "update --optional")
    assert result.returncode == 0
    status = git_fleximod(test_repo, f"status {repo_name}")
    ...
    assert (
        "fatal: couldn't find remote ref" in result.stderr
        or "error: pathspec 'somethingelse' did not match any file(s) known to git"
        in result.stderr
    )
